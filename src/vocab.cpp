...
bool mycompare(const vocab_word& a, const vocab_word& b) {
	return a.cn > b.cn;
}

void my_sort(struct vocab_word *data, int len, int nbThreads) {
	if (nbThreads<2) {
		std::sort(data, (&(data[len])), mycompare);
	}
	else {

		std::future<void> b = std::async(std::launch::async, my_sort, data, len / 2, nbThreads - 2);
		std::future<void> a = std::async(std::launch::async, my_sort, &data[len / 2], len % 2 == 0 ? len / 2 : len / 2 + 1, nbThreads - 2);

		a.wait();
		b.wait();

		std::inplace_merge(data, &data[len / 2], &data[len], mycompare);
	}
}
...
void SortAndReduceVocab(struct vocabulary *v, int min_count) {
  ...
  // Sort the vocabulary and keep </s> at the first position
	...
	my_sort(&(v->vocab[1]), v->vocab_size - 1, 24);
...
struct vocabulary *CreateVocabulary() {
   struct vocabulary *v = (vocabulary*)malloc(sizeof(struct vocabulary));
   long long a;
   v->vocab_max_size = 1000;
   v->vocab_size = 0;
